...
run_aver() {
    ...
    if [ "$VERBOSE" = true ]; then
        print_section "python3 \"$AVER_PATH\" --override-repo-boundary --location \"$TEST_DIR\" \"$@\""
        python3 "$AVER_PATH" --override-repo-boundary --location "$TEST_DIR" "$@"
    else
        python3 "$AVER_PATH" --override-repo-boundary --location "$TEST_DIR" "$@" 2>&1
    fi
}
...
setup_test_environment() {
    ...
    if run_aver admin config set-user --handle "test_user" --email "test@example.com" > /dev/null 2>&1; then
        pass
    else
...
test_basic_creation() {
    ...
    local output
    if output=$(run_aver record create --title "Test Record 1" 2>&1); then
    ...
    if output=$(run_aver record create --title "Test Record 2" severity#3 2>&1); then
        local rec_id=$(echo "$output" | grep -oE "REC-[A-Z0-9]+")
        if [ -n "$rec_id" ]; then
            pass
            echo "  Created: $rec_id"
    ...
    if output=$(run_aver record create --title "Test Record 3" --tags bug --tags urgent 2>&1); then
        local rec_id=$(echo "$output" | grep -oE "REC-[A-Z0-9]+")
        if [ -n "$rec_id" ]; then
            pass
            echo "  Created: $rec_id"
    ...
    if output=$(run_aver record create --id "CUSTOM-001" --title "Custom ID Test" 2>&1); then
...
test_special_characters() {
    ...
    if output=$(run_aver record create --title "Test #1" 2>&1); then
        local rec_id=$(echo "$output" | grep -oE "REC-[A-Z0-9]+")
        if [ -n "$rec_id" ]; then
    ...
    if output=$(run_aver record create --title "Cost: \$50" 2>&1); then
        local rec_id=$(echo "$output" | grep -oE "REC-[A-Z0-9]+")
        if [ -n "$rec_id" ]; then
    ...
    if output=$(run_aver record create --title "Success rate: 95%" 2>&1); then
        local rec_id=$(echo "$output" | grep -oE "REC-[A-Z0-9]+")
        if [ -n "$rec_id" ]; then
    ...
    if output=$(run_aver record create --title "Bug #42: Cost \$100 (50% off)" 2>&1); then
        local rec_id=$(echo "$output" | grep -oE "REC-[A-Z0-9]+")
        if [ -n "$rec_id" ]; then
            pass
        else
...
test_template_system() {
    ...
    set +e
    output=$(run_aver record create --template bug --title "Login broken" 2>&1)
    exit_code=$?
    set -e
    
    if echo "$output" | grep -q "requires editor mode"; then
        pass
...
test_validation() {
    ...
    set +e
    output=$(run_aver record create --status open 2>&1)
    exit_code=$?
    set -e
    ...
    set +e
    output=$(run_aver record create --title "Test" --status invalid_status 2>&1)
    exit_code=$?
    set -e
    ...
    if output=$(run_aver record create --title "Valid Status Test" --status open 2>&1); then
        pass
    else
    ...
    if output=$(run_aver record create --title "Default Test" 2>&1); then
        local rec_id=$(echo "$output" | grep -oE "REC-[A-Z0-9]+")
...
test_system_fields() {
    ...
    if output=$(run_aver record create --title "System Fields Test" 2>&1); then
        local rec_id=$(echo "$output" | grep -oE "REC-[A-Z0-9]+")
    ...
    if output=$(run_aver record create --title "User Field Test" 2>&1); then
        local rec_id=$(echo "$output" | grep -oE "REC-[A-Z0-9]+")
    ...
    if output=$(run_aver record create --title "Complete System Fields" 2>&1); then
        local rec_id=$(echo "$output" | grep -oE "REC-[A-Z0-9]+")
        if [ -n "$rec_id" ]; then
...
test_listing_search() {
    ...
    run_aver record create --title "Searchable 1" --status open --priority high > /dev/null 2>&1
    run_aver record create --title "Searchable 2" --status in_progress --priority low > /dev/null 2>&1
    run_aver record create --title "Searchable 3" --status closed --priority high > /dev/null 2>&1
    ...
    if output=$(run_aver record search --ksearch --status open 2>&1); then
    ...
    if output=$(run_aver record search --ksearch --priority high 2>&1); then
...
test_user_profile() {
    ...
    if run_aver admin config set-user --handle "testuser" --email "test@example.com" > /dev/null 2>&1; then
...
test_library_management() {
    ...
    python3 "$AVER_PATH" --location "$db1" admin init > /dev/null 2>&1
    python3 "$AVER_PATH" --location "$db2" admin init > /dev/null 2>&1
    python3 "$AVER_PATH" --location "$db3" admin init > /dev/null 2>&1
    
    # Set user for each database
    python3 "$AVER_PATH" --location "$db1" admin config set-user --handle "user1" --email "user1@example.com" > /dev/null 2>&1
    python3 "$AVER_PATH" --location "$db2" admin config set-user --handle "user2" --email "user2@example.com" > /dev/null 2>&1
    python3 "$AVER_PATH" --location "$db3" admin config set-user --handle "user3" --email "user3@example.com" > /dev/null 2>&1
    
    print_test "Add library alias 'work'"
    if python3 "$AVER_PATH" admin config add-alias --alias work --path "$db1" > /dev/null 2>&1; then
    ...
    if python3 "$AVER_PATH" admin config add-alias --alias personal --path "$db2" > /dev/null 2>&1; then
    ...
    if python3 "$AVER_PATH" admin config add-alias --alias archive --path "$db3" > /dev/null 2>&1; then
    ...
    if output=$(python3 "$AVER_PATH" admin config list-aliases 2>&1); then
    ...
    if python3 "$AVER_PATH" --use work record create --title "Work Record" --status open > /dev/null 2>&1; then
    ...
    if python3 "$AVER_PATH" --use personal record create --title "Personal Record" --status open > /dev/null 2>&1; then
    ...
    if python3 "$AVER_PATH" admin config remove-alias --alias archive > /dev/null 2>&1; then
        set +e
        if ! check_content_contains "$TEST_HOME/.config/aver/user.toml" "archive"; then
            pass
        else
...
test_custom_locations() {
    ...
    if python3 "$AVER_PATH" --location "$custom1" admin init > /dev/null 2>&1; then
        if [ -d "$custom1" ]; then
            # Set user for this location
            python3 "$AVER_PATH" --location "$custom1" admin config set-user --handle "custom1_user" --email "custom1@example.com" > /dev/null 2>&1
            pass
        else
    ...
    if python3 "$AVER_PATH" --location "$custom2" admin init > /dev/null 2>&1; then
        if [ -d "$custom2" ]; then
            # Set user for this location
            python3 "$AVER_PATH" --location "$custom2" admin config set-user --handle "custom2_user" --email "custom2@example.com" > /dev/null 2>&1
            pass
        else
            fail "Custom directory not created"
        fi
    ...
    if python3 "$AVER_PATH" --location "$custom1" record create --title "Custom Loc 1 Record" --status open > /dev/null 2>&1; then
    ...
    if python3 "$AVER_PATH" --location "$custom2" record create --title "Custom Loc 2 Record" --status open > /dev/null 2>&1; then
...
EOF
    ...
    local output1=$(python3 "$AVER_PATH" --location "$custom1" record create --title "Prefix Test 1" 2>&1)
    local output2=$(python3 "$AVER_PATH" --location "$custom2" record create --title "Prefix Test 2" 2>&1)
...
test_config_per_location() {
    ...
    # Initialize
    python3 "$AVER_PATH" --location "$loc_strict" admin init > /dev/null 2>&1
    python3 "$AVER_PATH" --location "$loc_relaxed" admin init > /dev/null 2>&1
    
    # Set user for each location
    python3 "$AVER_PATH" --location "$loc_strict" admin config set-user --handle "strict" --email "strict@example.com" > /dev/null 2>&1
    python3 "$AVER_PATH" --location "$loc_relaxed" admin config set-user --handle "relaxed" --email "relaxed@example.com" > /dev/null 2>&1
...
EOF
    ...
    set +e
    output=$(python3 "$AVER_PATH" --location "$loc_strict" record create --title "Test" 2>&1)
    exit_code=$?
    set -e
...
EOF
    ...
    if python3 "$AVER_PATH" --location "$loc_relaxed" record create --title "Test" > /dev/null 2>&1; then
        pass
...
test_updates() {
    ...
    local output=$(run_aver record create --title "Update Test" --status open 2>&1)
    local rec_id=$(echo "$output" | grep -oE "REC-[A-Z0-9]+")
    ...
    if run_aver record update "$rec_id" --status in_progress > /dev/null 2>&1; then
